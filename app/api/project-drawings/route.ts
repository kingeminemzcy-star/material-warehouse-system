...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ drawings: activeDrawings(meta.drawings, includeVoided) });
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (!body.projectId || !body.drawingId || !body.action) return NextResponse.json({ error: "缺少项目、图号或操作类型。" }, { status: 400 });
  if ((body.action === "void" || body.action === "restore") && !isAdminOrBoss(auth.profile.role)) return NextResponse.json({ error: "只有老板/管理员可以作废或恢复图号。" }, { status: 403 });
  if ((body.action === "update" || body.action === "void") && !body.reason?.trim()) return NextResponse.json({ error: "修改或作废图号必须填写原因。" }, { status: 400 });
  ...
  if (!oldDrawing) return NextResponse.json({ error: "图号不存在。" }, { status: 404 });
  ...
  return NextResponse.json({ message: body.action === "update" ? "图号已修改。" : body.action === "void" ? "图号已作废。" : "图号已恢复。", drawing: nextDrawing });
}

export async function DELETE(request: NextRequest) {
  const auth = await getAuthContext(request);
  if (!auth) return NextResponse.json({ error: "未登录或账号已禁用。" }, { status: 401 });
  if (!isAdminOrBoss(auth.profile.role)) return NextResponse.json({ error: "只有老板/管理员可以删除图号。" }, { status: 403 });
  const body = (await request.json()) as { projectId?: string; drawingId?: string; reason?: string };
  if (!body.projectId || !body.drawingId) return NextResponse.json({ error: "缺少项目或图号。" }, { status: 400 });
  if (!body.reason?.trim()) return NextResponse.json({ error: "删除或作废图号必须填写原因。" }, { status: 400 });
  const supabase = createSupabaseAdminClient();
  const { data: before, error: beforeError } = await supabase.from("Project").select("*").eq("id", body.projectId).maybeSingle();
  if (beforeError) return NextResponse.json({ error: beforeError.message }, { status: 500 });
  if (!before) return NextResponse.json({ error: "项目不存在。" }, { status: 404 });
  ...
  if (!drawing) return NextResponse.json({ error: "图号不存在。" }, { status: 404 });
  const now = new Date().toISOString();
  ...
  return NextResponse.json({ message: count > 0 ? "图号已作废，历史记录保留。" : "图号已删除。" });
}
