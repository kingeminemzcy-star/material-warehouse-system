    ...
    <div className="grid gap-6 xl:grid-cols-[430px_1fr]">
      <section className="rounded-lg border border-line bg-white p-4 shadow-soft">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-1">
          ...
          <label className="grid gap-2"><span className="form-label">项目图号</span><select className="field" value={form.drawingId} onChange={(e) => setForm({ ...form, drawingId: e.target.value })}><option value="">不关联图号</option>{drawings.map((d) => <option key={d.id} value={d.id}>{d.drawingNo} / {d.name} / {d.version}</option>)}</select></label>
      ...
      <section className="grid gap-3">
        ...
        <div className="hidden overflow-x-auto rounded-lg border border-line bg-white md:block"><table className="w-full min-w-[860px] text-left"><thead className="bg-field"><tr>{["申请单号","工程","申请人","材料","数量","用途","期望到货","状态"].map((c)=><th className="px-4 py-3 text-sm font-black" key={c}>{c}</th>)}</tr></thead><tbody className="divide-y divide-line">{requests.map((r)=><tr key={r.id}>{[r.requestNo,r.project,r.applicant,r.material,r.quantity,r.purpose,r.expected].map((v)=><td className="px-4 py-3 text-sm text-ink/75" key={v}>{v}</td>)}<td className="px-4 py-3"><StatusBadge status={r.statusText}/></td></tr>)}</tbody></table></div>
      </section>
    </div>
  );
}
