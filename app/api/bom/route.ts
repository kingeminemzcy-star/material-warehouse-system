...
export async function POST(request: NextRequest) {
  ...
  if (before.status === "COMPLETED") return NextResponse.json({ error: "项目已完工，只读状态下不能上传 BOM。" }, { status: 400 });
  ...
  return NextResponse.json({ message: "BOM 已上传并解析。", bom: { ...bom, analysis: analyzeBom(bom, stock) } });
}
...
export async function PATCH(request: NextRequest) {
  ...
  if (beforeError) return NextResponse.json({ error: beforeError.message }, { status: 500 });
  if (!before) return NextResponse.json({ error: "项目不存在。" }, { status: 404 });
  if (before.status === "COMPLETED") return NextResponse.json({ error: "项目已完工，只读状态下不能操作 BOM。" }, { status: 400 });
  const notes = parseProjectNotes(before.notes);
  const bom = notes.boms.find((item) => item.id === body.bomId);
  if (!bom) return NextResponse.json({ error: "BOM 不存在。" }, { status: 404 });
  const now = new Date().toISOString();
  ...
  if (shortageRows.length === 0) {
    return NextResponse.json({ message: skipped.length ? "存在未匹配材料，请先创建材料档案。" : "当前 BOM 无缺料，无需生成采购申请。", skipped });
  }
  ...
  return NextResponse.json({ message: `已生成 ${created.length} 条采购申请。`, created, skipped });
}
