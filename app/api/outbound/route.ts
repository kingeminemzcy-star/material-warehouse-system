...
export async function POST(request: NextRequest) {
  ...
  if (!["ADMIN", "BOSS", "WAREHOUSE", "PROJECT_MANAGER"].includes(auth.profile.role)) return NextResponse.json({ ok: false, error: "无出库权限。" }, { status: 403 });
  ...
  if (!body.projectId || !body.drawingId || !body.materialId || !body.specId || !body.zone || !body.locationCode || !body.quantity || !body.unit || !body.purpose) return NextResponse.json({ ok: false, error: "出库必须关联工程、图号，并填写材料、数量、用途。" }, { status: 400 });
  ...
  if (project.status === "COMPLETED") return NextResponse.json({ ok: false, error: "项目已完工，只读状态下不能新增出库。" }, { status: 400 });
  const drawings = parseProjectNotes(project.notes).drawings;
  if (!drawings.some((drawing) => drawing.id === body.drawingId)) {
    return NextResponse.json({ ok: false, error: "出库必须选择该项目下的有效图号。" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: true, message: "出库成功，库存已扣减。", ...result });
  } catch (error) {
    if (error instanceof Error && error.message.startsWith("库存不足")) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });
    throw error;
  }
}
